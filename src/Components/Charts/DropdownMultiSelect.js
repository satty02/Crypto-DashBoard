...
const DropdownMultiSelect = () => {
  ...
  return (
    <div className="relative">
      ...
      <button
        type="button"
        className="py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white flex items-center justify-between w-full"
        onClick={()=>setDropdownOpen(!dropdownOpen)}
      >
        <span>
          {selectedOptions.length === 0
            ? 'Select options'
            : `${selectedOptions.join(', ')} selected` }
        </span>
            ...
            >
              <label className="flex items-center">
                <input
                  ...
                  onChange={() => dispatch(coinSelectionAction(handleOptionChange(option.id)))}
                  
                />
                <span className="ml-2 text-gray-700">{option.id}</span>
              </label>
            </li>
          ))}
        </ul>
